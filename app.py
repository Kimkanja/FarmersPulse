...
def init_db():
    ...
    if not admin:
        ...
        conn.execute(
            "INSERT INTO users (username,email,password,role,full_name,county) VALUES (?,?,?,?,?,?)",
...
def inject_globals():
    ...
    if u:
        ...
        if u["role"] == "admin":
            pending_ai = 0  # AI posts are now auto-published
            unread_feedback = conn.execute(
                "SELECT COUNT(*) FROM feedback WHERE status='unread'"
            ).fetchone()[0]
        conn.close()
...
def login():
    ...
    if request.method == "POST":
        ...
        conn = get_db()
        u = conn.execute(
            "SELECT * FROM users WHERE (username=? OR email=?) AND is_active=1", (ident, ident)
        ).fetchone()
        conn.close()
        if u and check_password_hash(u["password"], pw):
            if u["is_banned"]:
                flash(f"Account suspended: {u['ban_reason'] or 'Policy violation'}", "error")
                return redirect(url_for("login"))
...
def register():
    ...
    crop_list = request.form.getlist("crop_types[]")
    crop_str = ",".join([c.strip() for c in crop_list if c.strip()])
    conn.execute(
        "INSERT INTO users (username,email,password,full_name,phone,county,crop_types,farm_size,role) VALUES (?,?,?,?,?,?,?,?,?)",
        (d["username"], d["email"], pw, d.get("full_name", ""), d.get("phone", ""),
         d.get("county", ""), crop_str, d.get("farm_size", ""), "farmer")
    )
    uid = conn.execute("SELECT id FROM users WHERE username=?", (d["username"],)).fetchone()["id"]
    conn.execute(
        "INSERT INTO notifications (user_id,content,notif_type,link) VALUES (?,?,?,?)",
        (uid, "Welcome to FarmersPulse!", "system", "/dashboard")
    )
    conn.commit()
    conn.close()
...
def forgot_password():
    if request.method == "POST":
        ...
        if u:
            ...
            conn.execute("DELETE FROM password_reset_tokens WHERE user_id=?", (u["id"],))
            conn.execute(
            ...
            conn.close()
            flash(
                f"Password reset link ready. Use this token to reset: {token[:20]}... "
                "(In production this would be emailed)", "info"
            )
...
def dashboard():
    ...
    # ── AI PERSONALIZATION ──
    user = conn.execute(
        "SELECT county, crop_types FROM users WHERE id=?", (uid,)
    ).fetchone()

    user_county = (user["county"] or "").strip().lower()
    user_crops = [c.strip().lower() for c in (user["crop_types"] or "").split(",") if c.strip()]

    ai_log = conn.execute(
        "SELECT post_ids_returned FROM ai_personalization_logs WHERE user_id=? ORDER BY created_at DESC LIMIT 1",
        (uid,)
    ).fetchone()
    ...
    if ai_log and not category and not search:
        ...
        if ranked_ids:
            ...
            filtered_posts = []
            for p in all_ranked:
                post_counties = (p["target_counties"] or "").lower()
                post_crops = (p["target_crops"] or "").lower()
                county_ok = (not post_counties) or (user_county in post_counties)
                crop_ok = True
                if post_crops:
                    crop_ok = any(c in post_crops for c in user_crops)
                if county_ok and crop_ok:
                    filtered_posts.append(p)

            id_to_post = {p["id"]: p for p in filtered_posts}
    ...
    if not using_ai_feed:
        ...
        if search:
            where.append("(title LIKE ? OR short_text LIKE ?)")
            params.extend([f"%{search}%", f"%{search}%"])

        where.append("(target_counties='' OR LOWER(target_counties) LIKE ?)")
        params.append(f"%{user_county}%")

        if user_crops:
            crop_conditions = []
            for crop in user_crops:
                crop_conditions.append("LOWER(target_crops) LIKE ?")
                params.append(f"%{crop}%")
            where.append("(" + " OR ".join(crop_conditions) + " OR target_crops='' )")

        ws = " AND ".join(where)
        total = conn.execute(f"SELECT COUNT(*) FROM posts WHERE {ws}", params).fetchone()[0]
        posts = conn.execute(
            f"SELECT * FROM posts WHERE {ws} ORDER BY is_pinned DESC, created_at DESC LIMIT ? OFFSET ?",
            params + [per_page, offset]
        ).fetchall()

    saved_ids = [r["post_id"] for r in conn.execute(
        "SELECT post_id FROM saved_posts WHERE user_id=?", (uid,)
    ).fetchall()]

    liked_map = {r["post_id"]: bool(r["is_like"]) for r in conn.execute(
        "SELECT post_id,is_like FROM likes WHERE user_id=?", (uid,)
    ).fetchall()}
    ...
    posts_data = [
        {"post": p, "likes": ps(p["id"])[0], "dislikes": ps(p["id"])[1], "comments": ps(p["id"])[2]}
        for p in posts
    ]
...
def post_detail(post_id):
    ...
    uid = session["user_id"]
    cr = conn.execute(
        "SELECT c.*,u.username,u.full_name,u.profile_pic FROM comments c JOIN users u ON c.user_id=u.id "
        "WHERE c.post_id=? AND c.parent_id IS NULL ORDER BY c.created_at DESC",
        (post_id,)
    ).fetchall()
    comments = [
        {
            ...
            "replies": conn.execute(
                "SELECT c.*,u.username,u.full_name,u.profile_pic FROM comments c JOIN users u ON c.user_id=u.id "
                "WHERE c.parent_id=? ORDER BY c.created_at",
                (c["id"],)
            ).fetchall()
        }
        for c in cr
    ...
    likes = conn.execute(
        "SELECT COUNT(*) FROM likes WHERE post_id=? AND is_like=1", (post_id,)
    ).fetchone()[0]
    dislikes = conn.execute(
        "SELECT COUNT(*) FROM likes WHERE post_id=? AND is_like=0", (post_id,)
    ).fetchone()[0]
    cc = conn.execute(
        "SELECT COUNT(*) FROM comments WHERE post_id=? AND parent_id IS NULL", (post_id,)
    ).fetchone()[0]
    conn.close()
    return render_template(
        "post_detail.html", post=post, comments=comments, saved_ids=saved_ids,
        liked_map=liked_map, likes=likes, dislikes=dislikes, comment_count=cc
    )
...
def like_post(post_id):
    ...
    conn = get_db()
    ex = conn.execute(
        "SELECT id,is_like FROM likes WHERE post_id=? AND user_id=?", (post_id, uid)
    ).fetchone()
    if ex:
        if ex["is_like"] == (1 if is_like else 0):
            conn.execute("DELETE FROM likes WHERE id=?", (ex["id"],))
        else:
            conn.execute("UPDATE likes SET is_like=? WHERE id=?", (1 if is_like else 0, ex["id"]))
    else:
        conn.execute(
            "INSERT INTO likes (post_id,user_id,is_like) VALUES (?,?,?)",
            (post_id, uid, 1 if is_like else 0)
        )
    conn.commit()
...
def delete_comment(cid):
    ...
    if c["user_id"] != uid and session.get("role") != "admin":
        conn.close()
        return jsonify({"error": "Unauthorized"}), 403
    conn.execute("DELETE FROM comments WHERE id=? OR parent_id=?", (cid, cid))
    conn.commit()
    conn.close()
...
def save_post(post_id):
    ...
    conn = get_db()
    ex = conn.execute(
        "SELECT id FROM saved_posts WHERE post_id=? AND user_id=?", (post_id, uid)
    ).fetchone()
    if ex:
        conn.execute("DELETE FROM saved_posts WHERE id=?", (ex["id"],))
        saved = False
    else:
        conn.execute("INSERT INTO saved_posts (post_id,user_id) VALUES (?,?)", (post_id, uid))
        saved = True
    conn.commit()
    conn.close()
...
def profile(user_id):
    ...
    if not user:
        ...
        return redirect(url_for("dashboard"))
    posts = conn.execute(
        "SELECT * FROM posts WHERE user_id=? AND is_published=1 ORDER BY created_at DESC", (user_id,)
    ).fetchall()
    ...
    is_following = bool(conn.execute(
        "SELECT id FROM follows WHERE follower_id=? AND followed_id=?", (session["user_id"], user_id)
    ).fetchone())
    ...
    total_likes = conn.execute(
        "SELECT COUNT(*) FROM likes l JOIN posts p ON l.post_id=p.id WHERE p.user_id=? AND l.is_like=1",
        (user_id,)
    ).fetchone()[0]
    conn.close()
    return render_template(
        "profile.html", user=user, posts_data=posts_data, followers=followers,
        following=following, is_following=is_following, saved_count=saved_count, total_likes=total_likes
    )
...
def saved_posts():
    ...
    conn = get_db()
    saves = conn.execute(
        "SELECT p.* FROM posts p JOIN saved_posts s ON p.id=s.post_id WHERE s.user_id=? ORDER BY s.created_at DESC",
        (uid,)
    ).fetchall()
    conn.close()
    return render_template("saved_posts.html", posts=saves)
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        crop_list = request.form.getlist("crop_types[]")
        crop_types = ",".join([c.strip() for c in crop_list if c.strip()])
        farm_size = request.form.get("farm_size", "")
        new_email = request.form.get("email", existing["email"])
        if new_email != existing["email"] and conn.execute(
        ...
        if "profile_pic" in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                ...
                fn = secure_filename(f"profile_{uid}_{file.filename}")
                file.save(os.path.join(UPLOAD_FOLDER, fn))
                pic = fn
        conn.execute(
            "UPDATE users SET full_name=?,bio=?,phone=?,county=?,crop_types=?,farm_size=?,email=?,profile_pic=? WHERE id=?",
            (full_name, bio, phone, county, crop_types, farm_size, new_email, pic, uid)
        )
...
def follow_user(user_id):
    ...
    conn = get_db()
    ex = conn.execute(
        "SELECT id FROM follows WHERE follower_id=? AND followed_id=?", (fid, user_id)
    ).fetchone()
    if ex:
        conn.execute("DELETE FROM follows WHERE id=?", (ex["id"],))
        following = False
    else:
        conn.execute("INSERT INTO follows (follower_id,followed_id) VALUES (?,?)", (fid, user_id))
        following = True
    conn.commit()
    count = conn.execute("SELECT COUNT(*) FROM follows WHERE followed_id=?", (user_id,)).fetchone()[0]
    conn.close()
...
def admin_dashboard():
    ...
    for i in range(6, -1, -1):
        ...
        lbl = (now - timedelta(days=i)).strftime("%a")
        ug.append({
            "day": lbl,
            "count": conn.execute("SELECT COUNT(*) FROM users WHERE DATE(created_at)=?", (d,)).fetchone()[0]
        })
        pg.append({
            "day": lbl,
            "count": conn.execute("SELECT COUNT(*) FROM posts WHERE DATE(created_at)=?", (d,)).fetchone()[0]
        })
    cs = conn.execute("SELECT category,COUNT(*) as c FROM posts GROUP BY category").fetchall()
    coy = conn.execute(
        "SELECT county,COUNT(*) as c FROM users WHERE role='farmer' GROUP BY county LIMIT 8"
    ).fetchall()
    logs = conn.execute("SELECT * FROM ai_logs ORDER BY created_at DESC LIMIT 5").fetchall()
    top = conn.execute(
        "SELECT p.*,COUNT(l.id) as lc FROM posts p LEFT JOIN likes l ON p.id=l.post_id AND l.is_like=1 "
        "GROUP BY p.id ORDER BY lc DESC LIMIT 5"
    ).fetchall()
    conn.close()
    return render_template(
        "admin_dashboard.html",
        total_users=tu, total_posts=tp, total_comments=tc,
        published_posts=pp, total_likes=tl, banned_users=bu,
        recent_users=ru, recent_posts=rp,
        user_growth=json.dumps(ug), post_growth=json.dumps(pg),
        cat_stats=json.dumps([{"label": r["category"] or "General", "value": r["c"]} for r in cs]),
        county_stats=json.dumps([{"label": r["county"] or "Unknown", "value": r["c"]} for r in coy]),
...
def toggle_user(uid):
    conn = get_db()
    u = conn.execute("SELECT is_active,role FROM users WHERE id=?", (uid,)).fetchone()
...
@admin_required
def ban_user(uid):
    reason = request.json.get("reason", "Policy violation")
    conn = get_db()
    u = conn.execute("SELECT is_banned,role FROM users WHERE id=?", (uid,)).fetchone()
    new_val = 0 if u["is_banned"] else 1
    rsn = reason if new_val else ""
    if u and u["role"] != "admin":
        conn.execute("UPDATE users SET is_banned=?,ban_reason=? WHERE id=?", (new_val, rsn, uid))
        conn.commit()
    conn.close()
    return jsonify({"banned": bool(new_val)})
...
def delete_user(uid):
    ...
    if u and u["role"] != "admin":
        ...
        conn.execute("DELETE FROM saved_posts WHERE user_id=?", (uid,))
        conn.commit()
    conn.close()
...
def admin_edit_user(uid):
    ...
    if request.method == "POST":
        conn.execute(
            "UPDATE users SET full_name=?,email=?,phone=?,county=?,crop_types=?,farm_size=?,role=?,is_active=? WHERE id=?",
            (request.form.get("full_name", ""), request.form.get("email", u["email"]),
             request.form.get("phone", ""), request.form.get("county", ""),
             request.form.get("crop_types", ""), request.form.get("farm_size", ""),
...
def flag_comment(cid):
    reason = request.json.get("reason", "Negative content")
    conn = get_db()
    conn.execute("UPDATE comments SET is_flagged=1,flag_reason=? WHERE id=?", (reason, cid))
    conn.commit()
    conn.close()
    return jsonify({"success": True})
...
def admin_account():
        ...
        if action == "update_self":
            ...
            if "profile_pic" in request.files:
                ...
                if f and f.filename and allowed_file(f.filename):
                    os.makedirs(UPLOAD_FOLDER, exist_ok=True)
                    fn = secure_filename(f"profile_{uid}_{f.filename}")
                    f.save(os.path.join(UPLOAD_FOLDER, fn))
                    pic = fn
            conn.execute(
                "UPDATE users SET full_name=?,email=?,phone=?,county=?,profile_pic=? WHERE id=?",
                (request.form.get("full_name", ""), request.form.get("email", ""),
                 request.form.get("phone", ""), request.form.get("county", ""), pic, uid)
            )
            npw = request.form.get("new_password", "")
            if npw:
            ...
            un = request.form.get("new_username", "").strip()
            em = request.form.get("new_email", "").strip()
            pw = request.form.get("new_password_admin", "")
            fn = request.form.get("new_full_name", "")
            if not un or not em or not pw:
                flash("All fields required.", "error")
            elif conn.execute("SELECT id FROM users WHERE username=?", (un,)).fetchone():
                flash("Username taken.", "error")
            elif conn.execute("SELECT id FROM users WHERE email=?", (em,)).fetchone():
                flash("Email used.", "error")
            else:
                conn.execute(
                    "INSERT INTO users (username,email,password,role,full_name) VALUES (?,?,?,?,?)",
                    (un, em, generate_password_hash(pw), "admin", fn)
                )
                conn.commit()
                flash(f"Admin '{un}' created.", "success")
        conn.close()
        return redirect(url_for("admin_account"))
    ...
    conn.close()
    return render_template("admin_account.html", me=me, admins=admins)
...
def admin_posts():
    ...
    total = conn.execute(f"SELECT COUNT(*) FROM posts WHERE {ws}", params).fetchone()[0]
    posts = conn.execute(
        f"SELECT * FROM posts WHERE {ws} ORDER BY created_at DESC LIMIT ? OFFSET ?",
        params + [per_page, offset]
    ).fetchall()
    ...
    total_pages = (total + per_page - 1) // per_page
    return render_template(
        "admin_posts.html", posts_data=posts_data, search=search, category=category,
        page=page, total_pages=total_pages, total=total
    )
...
def admin_create_post():
    if request.method == "POST":
        ...
        if "image_file" in request.files:
            ...
            if f and f.filename and allowed_file(f.filename):
                os.makedirs(UPLOAD_FOLDER, exist_ok=True)
                fn = secure_filename(f"post_{datetime.now().timestamp()}_{f.filename}")
                f.save(os.path.join(UPLOAD_FOLDER, fn))
                img = f"/static/uploads/{fn}"
        conn = get_db()
        conn.execute(
            "INSERT INTO posts (title,short_text,full_text,image_url,category,tags,target_crops,"
            "target_counties,is_published,is_pinned,user_id,source) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            (request.form["title"], request.form["short_text"], request.form.get("full_text", ""),
             img, request.form.get("category", "advisory"), request.form.get("tags", ""),
             request.form.get("target_crops", ""), request.form.get("target_counties", ""),
             1 if request.form.get("is_published") else 0,
             1 if request.form.get("is_pinned") else 0, session["user_id"], "manual")
        )
        conn.commit()
        conn.close()
...
def admin_edit_post(pid):
    ...
    if request.method == "POST":
        ...
        if "image_file" in request.files:
            ...
            if f and f.filename and allowed_file(f.filename):
                os.makedirs(UPLOAD_FOLDER, exist_ok=True)
                fn = secure_filename(f"post_{pid}_{f.filename}")
                f.save(os.path.join(UPLOAD_FOLDER, fn))
                img = f"/static/uploads/{fn}"
        conn.execute(
            "UPDATE posts SET title=?,short_text=?,full_text=?,image_url=?,category=?,tags=?,"
            "target_crops=?,target_counties=?,is_published=?,is_pinned=?,updated_at=datetime('now') WHERE id=?",
            (request.form["title"], request.form["short_text"], request.form.get("full_text", ""),
             img, request.form.get("category", "advisory"), request.form.get("tags", ""),
             request.form.get("target_crops", ""), request.form.get("target_counties", ""),
             1 if request.form.get("is_published") else 0,
             1 if request.form.get("is_pinned") else 0, pid)
        )
        conn.commit()
        conn.close()
...
def toggle_post(pid):
    conn = get_db()
    p = conn.execute("SELECT is_published FROM posts WHERE id=?", (pid,)).fetchone()
    new_val = 0 if p["is_published"] else 1
    conn.execute("UPDATE posts SET is_published=? WHERE id=?", (new_val, pid))
    conn.commit()
    conn.close()
    return jsonify({"published": bool(new_val)})
...
def admin_pending_posts():
    conn = get_db()
    posts = conn.execute(
        "SELECT * FROM posts WHERE is_published=0 AND source='ai_model' ORDER BY created_at DESC"
    ).fetchall()
    conn.close()
...
def approve_post(pid):
    conn = get_db()
    conn.execute("UPDATE posts SET is_published=1 WHERE id=?", (pid,))
    conn.commit()
    conn.close()
    return jsonify({"success": True})
...
def admin_reports():
    ...
    ceng = conn.execute(
        "SELECT p.category,COUNT(l.id) as c FROM posts p "
        "LEFT JOIN likes l ON p.id=l.post_id AND l.is_like=1 GROUP BY p.category"
    ).fetchall()
    mly = []
    for i in range(5, -1, -1):
        ...
        mly.append({
            ...
            "count": conn.execute(
                "SELECT COUNT(*) FROM users WHERE strftime('%Y-%m',created_at)=?", (ym,)
            ).fetchone()[0]
        })
    top = conn.execute(
        "SELECT p.title,p.category,COUNT(l.id) as lc,"
        "(SELECT COUNT(*) FROM comments WHERE post_id=p.id) as cc "
        "FROM posts p LEFT JOIN likes l ON p.id=l.post_id AND l.is_like=1 "
        "GROUP BY p.id ORDER BY lc DESC LIMIT 10"
    ).fetchall()
    conn.close()
    return render_template(
        "admin_reports.html",
        total_users=tu, active_users=au, banned_users=bu,
        total_posts=tp, ai_posts=ai, manual_posts=mp,
        total_likes=tl, total_comments=tc, total_saves=ts, flagged_comments=fc,
...
@admin_required
def export_report():
    rtype = request.args.get("type", "users")
    fmt = request.args.get("format", "csv")
    conn = get_db()
    if rtype == "users":
        rows = conn.execute(
            "SELECT id,username,email,full_name,county,crop_types,farm_size,phone,"
            "created_at,last_login,is_active,is_banned FROM users WHERE role='farmer'"
        ).fetchall()
        headers = ["ID", "Username", "Email", "Full Name", "County", "Crops",
                   "Farm Size", "Phone", "Joined", "Last Login", "Active", "Banned"]
    elif rtype == "posts":
        rows = conn.execute(
            "SELECT p.id,p.title,p.category,p.source,p.is_published,"
        ...
        rows = conn.execute(
            ...
            "(SELECT COUNT(*) FROM comments WHERE post_id=p.id) as comments,"
            "(SELECT COUNT(*) FROM saved_posts WHERE post_id=p.id) as saves FROM posts p"
        ).fetchall()
        headers = ["Title", "Category", "Likes", "Dislikes", "Comments", "Saves"]
    elif rtype == "comments":
        rows = conn.execute(
            "SELECT c.id,u.username,p.title,c.content,c.is_flagged,c.flag_reason,c.created_at "
    ...
    if fmt == "csv":
        ...
        return send_file(
            ...
            download_name=f"fp_{rtype}_{ts}.csv"
        )
    elif fmt == "json":
        ...
        return send_file(
            ...
            download_name=f"fp_{rtype}_{ts}.json"
        )
    elif fmt == "html":
        rh = "".join(
            f"<tr>{''.join(f'<td>{v}</td>' for v in r.values())}</tr>" for r in data
        )
        hh = "".join(f"<th>{h}</th>" for h in headers)
        html = f"""<!DOCTYPE html><html><head><meta charset='UTF-8'>
        <title>FarmersPulse – {rtype} Report</title>
        <style>
            body {{ font-family: 'Times New Roman', serif; padding: 32px; }}
            h1 {{ color: #2e7d32; }}
            h2 {{ color: #ff7f0e; font-size: 16px; }}
            table {{ width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 13px; }}
            th {{ background: #2e7d32; color: #fff; padding: 8px 10px; text-align: left; }}
            td {{ padding: 7px 10px; border-bottom: 1px solid #dde8dd; }}
            tr:nth-child(even) {{ background: #f3f7f3; }}
            @media print {{ .noprint {{ display: none; }} }}
        </style></head>
        <body>
            <h1>FarmersPulse – {rtype.title()} Report</h1>
            <h2>Generated: {datetime.now().strftime('%B %d, %Y %H:%M')}</h2>
            <p>Total records: {len(data)}</p>
            <button class='noprint' onclick='window.print()'
                style='background:#2e7d32;color:#fff;border:none;padding:8px 20px;
                       border-radius:6px;cursor:pointer;margin-bottom:16px;'>
                Print / Save PDF
            </button>
            <table><thead><tr>{hh}</tr></thead><tbody>{rh}</tbody></table>
        </body></html>"""
        resp = make_response(html)
        resp.headers["Content-Type"] = "text/html; charset=utf-8"
        resp.headers["Content-Disposition"] = f"attachment; filename=fp_{rtype}_{ts}.html"
        return resp
...
def api_receive_post():
    ...
    if not data or not data.get("title") or not data.get("short_text"):
        return jsonify({"error": "title and short_text required"}), 400
    conn = get_db()
    admin = conn.execute("SELECT id FROM users WHERE role='admin'").fetchone()
    conn.execute(
        "INSERT INTO posts (title,short_text,full_text,image_url,category,tags,"
        "target_crops,target_counties,is_published,source,user_id) VALUES (?,?,?,?,?,?,?,?,1,?,?)",
        (data["title"], data["short_text"], data.get("full_text", ""), data.get("image_url", ""),
         data.get("category", "advisory"), data.get("tags", ""),
         data.get("target_crops", ""), data.get("target_counties", ""),
         "ai_model", admin["id"] if admin else 1)
    )
    pid = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
        "INSERT INTO ai_logs (endpoint,payload,response_status,post_id) VALUES (?,?,?,?)",
        ("/api/ai/post", json.dumps(data), "auto_published", pid)
    )
    if admin:
        conn.execute(
            "INSERT INTO notifications (user_id,content,notif_type,link) VALUES (?,?,?,?)",
            (admin["id"], f"AI post auto-published: {data['title'][:50]}", "system", f"/post/{pid}")
        )
    conn.commit()
    conn.close()
    return jsonify({
        "success": True, "post_id": pid,
        "status": "published", "message": "Post auto-published in real time"
    }), 201
...
def api_get_user_profile(uid):
    ...
    conn = get_db()
    u = conn.execute(
        "SELECT id,username,full_name,county,crop_types,farm_size,created_at,last_login "
        "FROM users WHERE id=? AND role='farmer'", (uid,)
    ).fetchone()
    if not u:
        conn.close()
        return jsonify({"error": "User not found"}), 404
    liked_posts = conn.execute(
        "SELECT p.id,p.category,p.tags,p.target_crops,p.target_counties "
        "FROM likes l JOIN posts p ON l.post_id=p.id "
        "WHERE l.user_id=? AND l.is_like=1 ORDER BY l.created_at DESC LIMIT 20", (uid,)
    ).fetchall()
    saved_posts = conn.execute(
        "SELECT p.id,p.category,p.tags FROM saved_posts s JOIN posts p ON s.post_id=p.id "
        "WHERE s.user_id=? ORDER BY s.created_at DESC LIMIT 20", (uid,)
    ).fetchall()
    commented = conn.execute(
        "SELECT DISTINCT p.id,p.category FROM comments c JOIN posts p ON c.post_id=p.id "
    ...
    return jsonify({
        ...
        "interaction_history": {
            "liked_posts": [
                {"id": p["id"], "category": p["category"], "tags": p["tags"],
                 "crops": p["target_crops"], "counties": p["target_counties"]}
...
def api_get_all_posts():
    ...
    posts = conn.execute(
        "SELECT id,title,short_text,category,tags,target_crops,target_counties,"
    ...
    for p in posts:
        likes = conn.execute(
            "SELECT COUNT(*) FROM likes WHERE post_id=? AND is_like=1", (p["id"],)
        ).fetchone()[0]
...
def api_set_personalized_feed():
    ...
    conn.execute(
        "INSERT INTO ai_personalization_logs "
        "(user_id,request_payload,response_payload,post_ids_returned,response_time_ms) "
        "VALUES (?,?,?,?,?)",
...
def api_bulk_profiles():
    ...
    conn = get_db()
    users = conn.execute(
        "SELECT id,username,county,crop_types,farm_size,last_login "
        "FROM users WHERE role='farmer' AND is_active=1 AND is_banned=0"
    ).fetchall()
    conn.close()
    return jsonify({
        "total": len(users),
...
def api_ai_report():
    """Read-only report endpoint for the AI model to check system health and stats."""
    if request.headers.get("X-API-Key", "") != AI_API_KEY:
        return jsonify({"error": "Unauthorized"}), 401
    conn = get_db()
    total_ai_posts = conn.execute("SELECT COUNT(*) FROM posts WHERE source='ai_model'").fetchone()[0]
    published_today = conn.execute(
        "SELECT COUNT(*) FROM posts WHERE source='ai_model' AND DATE(created_at)=DATE('now')"
    ).fetchone()[0]
    total_users = conn.execute(
        "SELECT COUNT(*) FROM users WHERE role='farmer' AND is_active=1 AND is_banned=0"
    ).fetchone()[0]
    personalized = conn.execute(
        "SELECT COUNT(DISTINCT user_id) FROM ai_personalization_logs"
    ).fetchone()[0]
    recent_logs = conn.execute(
        "SELECT endpoint,response_status,post_id,created_at FROM ai_logs ORDER BY created_at DESC LIMIT 10"
    ).fetchall()
    top_cats = conn.execute(
        "SELECT category,COUNT(*) as c FROM posts WHERE source='ai_model' GROUP BY category ORDER BY c DESC"
    ).fetchall()
    recent_posts = conn.execute(
        "SELECT id,title,category,created_at FROM posts WHERE source='ai_model' ORDER BY created_at DESC LIMIT 5"
    ).fetchall()
    conn.close()
    return jsonify({
        "status": "ok",
        ...
        "summary": {
            "total_ai_posts_published": total_ai_posts,
            "published_today": published_today,
            "active_farmers": total_users,
            "farmers_with_personalized_feed": personalized,
        },
        "category_breakdown": [
            {"category": r["category"] or "general", "count": r["c"]} for r in top_cats
        ],
        "recent_activity": [
            {"endpoint": r["endpoint"], "status": r["response_status"],
             "post_id": r["post_id"], "at": r["created_at"][:16]}
            for r in recent_logs
        ],
        "latest_ai_posts": [
            {"id": p["id"], "title": p["title"], "category": p["category"],
             "published_at": p["created_at"][:16]}
...
def feedback():
    if request.method == "POST":
        conn = get_db()
        conn.execute(
            "INSERT INTO feedback (user_id,name,email,subject,message,category) VALUES (?,?,?,?,?,?)",
            (session["user_id"], request.form.get("name", ""), request.form.get("email", ""),
        ...
        if admin:
            conn.execute(
                "INSERT INTO notifications (user_id,content,notif_type,link) VALUES (?,?,?,?)",
                (admin["id"],
                 f"New feedback from {session['username']}: {request.form.get('subject', '')[:50]}",
...
def admin_feedback():
    ...
    feedbacks = conn.execute(
        f"SELECT f.*,u.username FROM feedback f LEFT JOIN users u ON f.user_id=u.id "
        f"WHERE {where} ORDER BY f.created_at DESC", params
    ).fetchall()
    counts = {
    ...
    conn.close()
    return render_template(
        "admin_feedback.html", feedbacks=feedbacks, counts=counts, status_filter=status_filter
    )
